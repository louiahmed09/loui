...
def start(message):
	...
	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/K_F_P')
		mas.add(G,K)
		mas.add(V)
		mas.add(M)
		...
		me = types.InlineKeyboardButton(text="مجهول",url="https://t.me/K_F_P")
		...
		ch = types.InlineKeyboardButton(text="▶ قناة البوت ◀",url="https://t.me/K_F_P")
		rr.add(me,he,de)
		rr.add(ch)
...
def masg(call):
	...
	if call.data =="K_F_P":
		...
		while True:
			...
			K_F_P = [h1,h2,h3,h4,h5]
			j = random.choice(K_F_P)
			url = "https://t.me/"+j
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				R = types.InlineKeyboardButton(f'{sk}', callback_data="1x")
				K_F_P = types.InlineKeyboardButton('DEV', url='https://t.me/K_F_P')
				mas.add(A,E,B,R,K_F_P)
				bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="ok start",reply_markup=mas)
